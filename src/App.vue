  ...
  <div class="pdf-mindmap-plugin">
    ...
    <div class="panel-header">
      ...
      <div class="header-right">
        ...
        <button
          class="header-btn icon-btn close-btn"
          title="关闭"
          @click="handleClose"
        >
          <svg
    ...
    <!-- 工具栏 -->
    <div
      v-if="currentLearningSet && currentPdf"
      class="toolbar"
    >
      <div class="toolbar-left">
        ...
        >
          <option
            v-for="pdf in currentLearningSet.pdfs"
            :key="pdf.id"
            :value="pdf.id"
          >
            {{ pdf.name }}
      ...
      <div class="toolbar-center">
        <div class="toolbar-group">
          ...
          >
            <option
              v-for="level in ANNOTATION_LEVELS"
              :key="level.value"
              :value="level.value"
            >
              {{ level.label }}
    ...
    >
      ...
      <div
        v-if="viewMode === 'split'"
        class="annotation-area mindmap-sidebar-area"
        :style="{ width: `${annotationWidth}px` }"
      >
        <div
          v-if="currentLearningSet && currentPdf"
          class="mindmap-sidebar-compact"
        >
          <!-- 头部工具栏 -->
          <div class="mindmap-sidebar-header">
            ...
            <div class="mindmap-header-actions">
              <button
                class="action-btn"
                :title="mindMapAutoSync ? '自动同步已开启' : '自动同步已关闭'"
                @click="toggleMindMapAutoSync"
              >
          ...
          <!-- 画布区域 -->
          <div
            class="mindmap-sidebar-body"
            :data-debug="`mindMapBlockId=${mindMapBlockId}`"
          >
            <div
              v-if="!mindMapBlockId"
        ...
        >
          ...
          <button
            class="b3-button b3-button--primary"
            @click="showLearningSetManager = true"
          >
            选择学习集
          </button>
        </div>
    ...
    <!-- 批量移动对话框 -->
    <BatchMoveDialog
      :visible="showBatchMoveDialog"
      :x="batchMoveDialogPosition.x"
      :y="batchMoveDialogPosition.y"
      :annotation-ids="batchMoveAnnotationIds"
      @update:visible="showBatchMoveDialog = $event"
      @success="handleBatchMoveSuccess"
      @cancel="handleBatchMoveCancel"
    />
    ...
    <!-- 复习会话 -->
    <div
      v-if="showReviewSession"
      class="side-panel-overlay"
      @click.self="showReviewSession = false"
    >
      <div class="side-panel-content">
        <ReviewSession
          :study-set-id="currentLearningSet?.id"
    ...
    <!-- 智能学习集 -->
    <div
      v-if="showSmartStudySet"
      class="side-panel-overlay"
      @click.self="showSmartStudySet = false"
    >
      <div class="side-panel-content">
        <SmartStudySet
          :cards="allCards"
    ...
    <!-- 第五阶段高级功能面板 -->
    <div
      v-if="showContextView"
      class="side-panel-overlay"
      @click.self="showContextView = false"
    >
      <div class="side-panel-content">
        <ContextView
    ...
    <div
      v-if="showKeywordLibrary"
      class="side-panel-overlay"
      @click.self="showKeywordLibrary = false"
    >
      <div class="side-panel-content">
        <KeywordLibrary
          :cards="allCards"
    ...
    <div
      v-if="showMemoryContext"
      class="side-panel-overlay"
      @click.self="showMemoryContext = false"
    >
      <div class="side-panel-content">
        <MemoryContext
          :cards="allCards"
    ...
    <div
      v-if="showFreeMindMap"
      class="side-panel-overlay free-mindmap-overlay"
      @click.self="showFreeMindMap = false"
    >
      <div class="free-mindmap-panel">
        <div class="free-mindmap-header">
          <span class="free-mindmap-title">🧠 自由画布思维导图</span>
          <button
            class="free-mindmap-close"
            title="关闭"
            @click="showFreeMindMap = false"
          >
            <svg
              viewBox="0 0 24 24"
              width="16"
    ...
    <div
      v-if="showPdfMindMapLink"
      class="side-panel-overlay pdf-mindmap-link-overlay"
      @click.self="showPdfMindMapLink = false"
    >
      <div class="pdf-mindmap-link-panel">
        <div class="link-panel-header">
          <span class="link-panel-title">📖 PDF+🧠 思维导图联动</span>
          <button
            class="link-panel-close"
            title="关闭"
            @click="showPdfMindMapLink = false"
          >
            <svg
              viewBox="0 0 24 24"
              width="16"
    ...
    <div
      v-if="newSetDialog.visible"
      class="dialog-overlay"
      @click.self="newSetDialog.visible = false"
    >
      <div class="dialog-panel">
          ...
          <div class="form-group">
            ...
            <div class="pdf-add-options">
              <button
                ...
                @click="selectPdfForNewSet"
              >
                {{ newSetDialog.pdfName || '选择 PDF 文件' }}
              </button>
              <span class="pdf-hint">或从思源工作空间选择</span>
              <select
                v-model="newSetDialog.workspacePdfPath"
                class="b3-text-field pdf-select-workspace"
              >
                <option value="">
                  -- 从工作空间选择 PDF --
                </option>
                <option
                  v-for="pdf in workspacePdfs"
                  :key="pdf.path"
                  :value="pdf.path"
                >
                  {{ pdf.name }}
        ...
        <div class="dialog-footer">
          <button
            class="b3-button b3-button--outline"
            @click="newSetDialog.visible = false"
          >
            取消
          </button>
          <button
...
const quickCaptureMenuPosition = ref({
  x: 0,
  y: 0,
})
...
const _loadingAnnotations = ref(false)
...
const batchMoveAnnotationIds = ref<string[]>([])
const batchMoveDialogPosition = ref({
  x: 0,
  y: 0,
})
...
const _handleAnnotationEdit = (ann: PDFAnnotation) => {
...
// 批量移动处理函数
const _handleBatchMoveRequest = (annotationIds: string[]) => {
  batchMoveAnnotationIds.value = annotationIds
  // 计算对话框位置（屏幕中央）
  const rect = document.getElementById('plugin-pdf-mindmap-panel')?.getBoundingClientRect()
  if (rect) {
    batchMoveDialogPosition.value = {
...
const handleBatchMoveSuccess = async () => {
  showMessage('批量移动成功', 'success')
  // 重新加载学习集数据
  await loadLearningSets()
  // 清除选中状态
  batchMoveAnnotationIds.value = []
...
const _handleAnnotationMerge = (sourceId: string, targetId: string) => {
  if (!currentLearningSet.value) return
...
const _handleAnnotationUnmerge = (annotationId: string) => {
  if (!currentLearningSet.value) return
...
const _handleCursorChange = (afterId: string | null) => {
    ...
    isDuplicate = deduplicatedAnnotations.some((a) => {
      ...
      if (a.text === newAnnotation.text
        ...
        if (a.rect && newAnnotation.rect) {
          ...
          const centerDist = Math.sqrt(((ax1 + ax2) / 2 - (bx1 + bx2) / 2) ** 2 + ((ay1 + ay2) / 2 - (by1 + by2) / 2) ** 2)
          if (centerDist < 10) {
            return true
          }
        }
...
// 显示消息提示（在插件容器内显示，不操作宿主 DOM）- 已移动到 utils
// const showMessage moved to avoid duplicate declaration
...
const handleImageSelected = async (data: {
  ...
  try {
    ...
    const cropX = Math.max(0, Math.round(data.canvasRect.x * scaleX))
    const cropY = Math.max(0, Math.round(data.canvasRect.y * scaleY))
...
const getCreateAnnotationLock = () => {
  ...
  (window as any).__PDF_CREATE_ANNOTATION_LOCK__ ||= {
    locked: false,
    text: '',
    level: '',
    time: 0,
