...
export class LearningSetApi {
   ...
   * 创建新的学习集
   */
  static async createLearningSet(name: string, description: string = ''): Promise<LearningSet | null> {
    try {
   ...
   * 向学习集中添加PDF
   */
  static async addPdfToLearningSet(setId: string, pdfPath: string): Promise<boolean> {
    try {
      const sets = await this.getAllLearningSets();
