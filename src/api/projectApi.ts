 ...
 */
let lastFetchTime = 0;
let lastFetchedData: ProjectStorage | null = null;
const FETCH_COOLDOWN = 2000; // 2秒冷却时间，避免频繁读取

async function getProjectStorageAsync(): Promise<ProjectStorage> {
  const now = Date.now();

  // 如果在冷却时间内，返回上次的数据
  if (lastFetchedData && (now - lastFetchTime) < FETCH_COOLDOWN) {
  ...
  // 如果没有缓存数据，返回默认值
  if (!lastFetchedData) {
    lastFetchedData = { projects: [], currentProjectId: null };
  }
  lastFetchTime = now;
  return lastFetchedData;
}
