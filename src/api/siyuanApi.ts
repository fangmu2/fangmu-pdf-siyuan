...
/**
 * 插件数据内存缓存
 * 用于避免 PUT 后立即 GET 的冗余请求
 */
const pluginDataCache = new Map<string, { data: any; timestamp: number }>();
const PLUGIN_DATA_CACHE_TTL = 60 * 1000; // 缓存有效期 60 秒

/**
 * 获取插件持久化数据（使用文件存储）
 * 数据存储在 /data/storage/petal/fangmu-pdf-siyuan/ 目录下
 * 优先使用内存缓存，避免重复 GET 请求
 */
export async function getPluginData<T = any>(key: string): Promise<T | null> {
  // 先检查内存缓存
  const cached = pluginDataCache.get(key);
  if (cached) {
    const now = Date.now();
    // 缓存有效期内，直接返回缓存数据
    if (now - cached.timestamp < PLUGIN_DATA_CACHE_TTL) {
      return cached.data as T;
    } else {
      // 缓存过期，清除
      pluginDataCache.delete(key);
    }
  }
  ...
  try {
    ...
    if (text) {
      ...
      // 存入内存缓存
      pluginDataCache.set(key, { data, timestamp: Date.now() });
      return data as T;
    }
    return null;
  } catch (e) {
...
export async function setPluginData<T>(key: string, value: T): Promise<boolean> {
  // 【关键优化】立即更新内存缓存，这样后续的 getPluginData 就不需要 GET 请求了
  pluginDataCache.set(key, { data: value, timestamp: Date.now() });
