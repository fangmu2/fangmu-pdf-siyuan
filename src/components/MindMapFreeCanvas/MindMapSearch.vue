<template>
  ...
  >
    ...
    >
      <div class="results-header">
        <span class="results-count">
          ...
          <template v-else>
            {{ t('mindmap.search.results', {
              count: searchStats?.uniqueNodes || 0, total: searchStats?.totalMatches || 0,
            }) }}
        ...
        >
          ...
          <div class="result-meta">
            <span
              v-if="match.field"
              class="field-tag"
            >{{ t(`mindmap.search.fields.${match.field}`) }}</span>
            <span
              v-if="match.tags?.length"
              class="tags"
            >
              <span
                v-for="tag in match.tags.slice(0, 3)"
                :key="tag"
                class="tag"
              >#{{ tag }}</span>
            </span>
    ...
    <!-- 空状态 -->
    <div
      v-if="!hasActiveSearch && !showHistory"
      class="empty-state"
    >
      <svg
        class="empty-icon"
