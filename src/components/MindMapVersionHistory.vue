  ...
  <div class="mindmap-version-history">
    ...
    <div class="version-header">
      ...
      <div class="version-actions">
        <button @click="createNewVersion" class="btn-primary" :disabled="!mindMapId">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
            <path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/>
          </svg>
          保存版本
        </button>
        <button @click="toggleAutoSave" :class="['btn-toggle', { active: autoSaveEnabled }]">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
    ...
    <div class="version-list">
      <div v-if="versions.length === 0" class="empty-state">
        ...
        <p>暂无版本历史</p>
        <p class="hint">点击"保存版本"创建第一个版本</p>
      </div>
      ...
      >
        ...
        <div class="version-info">
          ...
          <div class="version-meta">
            ...
            <span v-if="version.description" class="version-desc">{{ version.description }}</span>
          </div>
          <div v-if="version.changes.length > 0" class="version-changes">
            <span v-if="getChangeCount(version, 'add') > 0" class="change-badge add">
              +{{ getChangeCount(version, 'add') }}
            </span>
            <span v-if="getChangeCount(version, 'remove') > 0" class="change-badge remove">
              -{{ getChangeCount(version, 'remove') }}
            </span>
            <span v-if="getChangeCount(version, 'update') > 0" class="change-badge update">
              ~{{ getChangeCount(version, 'update') }}
            </span>
          </div>
        </div>
        <div class="version-actions-item">
          <button @click.stop="restoreVersion(version)" class="btn-icon" title="恢复此版本">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
              <path d="M12 5V1L7 6l5 5V7c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6H4c0 4.42 3.58 8 8 8s8-3.58 8-8-3.58-8-8-8z"/>
            </svg>
          </button>
          <button @click.stop="compareWithCurrent(version)" class="btn-icon" title="与当前对比">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
              <path d="M10 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h5v2h2V1h-2v2zm0 15H5l5-6v6zm9-15h-5v2h5v13l-5-6v9h5c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z"/>
            </svg>
          </button>
          <button @click.stop="deleteVersion(version)" class="btn-icon danger" title="删除版本">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
    ...
    <div v-if="showDiffPanel" class="diff-panel">
      <div class="diff-header">
        <h4>版本对比</h4>
        <button @click="closeDiffPanel" class="btn-close">×</button>
      </div>
      <div class="diff-content">
        ...
        <div class="diff-details">
          <div v-for="item in diffResult?.added" :key="item.nodeId" class="diff-row add">
            <span class="diff-type">+ 新增</span>
            <span class="diff-path">{{ item.path.join(' > ') }}</span>
          </div>
          <div v-for="item in diffResult?.removed" :key="item.nodeId" class="diff-row remove">
            <span class="diff-type">- 删除</span>
            <span class="diff-path">{{ item.path.join(' > ') }}</span>
          </div>
          <div v-for="item in diffResult?.modified" :key="item.nodeId" class="diff-row update">
