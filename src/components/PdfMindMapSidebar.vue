  ...
  >
    ...
    >
      ...
      <div class="sidebar-body">
        ...
        <!-- 无数据提示（非空状态） -->
        <div
          v-else-if="!showEmptyState && (!nodes || nodes.length === 0)"
          class="empty-hint"
        >
          <svg
            viewBox="0 0 24 24"
...
async function importExistingAnnotations() {
  ...
  // 如果没有标注数据，检查是否已有节点
  let annotationsToImport = props.annotations
  if (!annotationsToImport || annotationsToImport.length === 0) {
    console.log('[PdfMindMapSidebar] props 中没有标注数据，尝试从思源加载...')
    // 主动从思源加载 PDF 标注
    try {
      const { getAnnotationsForPdf } = await import('@/api/annotationApi')
      annotationsToImport = await getAnnotationsForPdf(props.pdfDocId) || []
      console.log('[PdfMindMapSidebar] 从思源加载到标注数量:', annotationsToImport.length)
    } catch (error) {
      console.error('[PdfMindMapSidebar] 加载标注失败:', error)
      annotationsToImport = []
    }
  }

  if (!annotationsToImport || annotationsToImport.length === 0) {
    console.log('[PdfMindMapSidebar] 确实没有标注数据')
  ...
  try {
    ...
    // 过滤未导入的摘录 - 使用 annotationsToImport 而不是 props.annotations
    const newAnnotations = (annotationsToImport || []).filter(
...
async function initMindMapBlockId() {
  ...
  try {
    ...
    if (!savedBlockId) {
      ...
      console.log('[PdfMindMapSidebar] 创建新的思维导图块 ID:', savedBlockId)
    }
...
onMounted(async () => {
  ...
  if (savedWidth) {
    currentWidth.value = Number.parseInt(savedWidth, 10)
