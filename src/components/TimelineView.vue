  ...
  <div class="timeline-view">
    ...
    <div class="timeline-toolbar">
      <div class="toolbar-group">
        <button
          :class="['toolbar-btn', { active: viewMode === 'month' }]"
        ...
        <button
          :class="['toolbar-btn', { active: viewMode === 'week' }]"
          @click="viewMode = 'week'"
          title="按周视图"
        >
          周
        ...
        <button
          :class="['toolbar-btn', { active: viewMode === 'day' }]"
          @click="viewMode = 'day'"
          title="按日视图"
        >
          日
      ...
      <div class="toolbar-group">
        <button @click="prevPeriod" class="toolbar-btn" title="上一个周期">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
            <path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/>
          </svg>
        </button>
        <button @click="nextPeriod" class="toolbar-btn" title="下一个周期">
          <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
            <path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/>
          </svg>
        </button>
        <button @click="goToToday" class="toolbar-btn toolbar-btn-primary" title="回到今天">
          今天
        </button>
      </div>
    ...
    <!-- 时间线内容 -->
    <div class="timeline-content" ref="contentRef">
      <!-- 月视图 -->
      <div v-if="viewMode === 'month'" class="timeline-month-view">
        <div
        ...
        >
          <div class="timeline-month__header">
            <h3 class="timeline-month__title">{{ month.label }}</h3>
          ...
          <div class="timeline-month__cards">
            <div
              ...
              :key="card.id"
              :class="['timeline-card', `timeline-card--${card.type}`]"
              @click="handleCardClick(card)"
            >
              <div class="timeline-card__date">{{ formatDate(card.created) }}</div>
              <div class="timeline-card__content">{{ card.content || card.front }}</div>
              <div class="timeline-card__meta">
                <span v-if="card.sourcePdf" class="timeline-card__pdf">
                  📄 {{ getPdfName(card.sourcePdf) }}
                </span>
                <span v-if="card.tags?.length" class="timeline-card__tags">
                  <span v-for="tag in card.tags" :key="tag" class="timeline-card__tag">
                    #{{ tag }}
                  </span>
                </span>
              </div>
      ...
      <!-- 周视图 -->
      <div v-else-if="viewMode === 'week'" class="timeline-week-view">
        <div
        ...
        >
          <div class="timeline-week__header">
            <h3 class="timeline-week__title">{{ week.label }}</h3>
      ...
      <!-- 日视图 -->
      <div v-else-if="viewMode === 'day'" class="timeline-day-view">
        <div
          v-for="day in dayData"
          :key="day.label"
          :class="['timeline-day', { 'timeline-day--today': day.isToday }]"
        >
          <div class="timeline-day__header">
            <div class="timeline-day__info">
              <h3 class="timeline-day__title">{{ day.label }}</h3>
          ...
          <div class="timeline-day__cards">
            <div
              ...
              @click="handleCardClick(card)"
            >
              <div class="timeline-card__time">{{ formatTime(card.created) }}</div>
              <div class="timeline-card__content">{{ card.content || card.front }}</div>
              <div class="timeline-card__meta">
                <span v-if="card.sourcePdf" class="timeline-card__pdf">
                  📄 {{ getPdfName(card.sourcePdf) }}
      ...
      <div v-if="allCards.length === 0" class="empty-state">
        ...
        <p>暂无内容</p>
        <p class="empty-state-hint">创建的卡片将按时间顺序显示在这里</p>
      </div>
    </div>
  </div>
</template>
