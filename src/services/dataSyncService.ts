...
class DataSyncServiceClass {
  ...
  async createFullBackup(config: Partial<BackupConfig> = {}): Promise<BackupMetadata> {
    const finalConfig: BackupConfig = {
      ...this.defaultConfig,
      ...config,
    }
  ...
  async createIncrementalBackup(
    ...
    try {
      ...
      studySetChanges.forEach((c) => changes.push({
        ...c,
        dataType: 'studySet',
      }))
      cardChanges.forEach((c) => changes.push({
        ...c,
        dataType: 'card',
      }))
      mindmapChanges.forEach((c) => changes.push({
        ...c,
        dataType: 'mindmap',
      }))
  ...
  async getCloudSyncConfig(): Promise<CloudSyncConfig> {
    try {
      const content = await getPluginData<CloudSyncConfig>('syncConfig')
      return content || this.defaultCloudConfig
    } catch {
  ...
  async saveCloudSyncConfig(config: CloudSyncConfig): Promise<void> {
    await setPluginData('syncConfig', config)
  }
  ...
  private async readFile(path: string): Promise<string> {
    return await getPluginData<string>(path) || ''
  }
