...
function getFSRSStateFromBlock(block: any): CardState {
  ...
  // 回退到 SM-2 格式
  const easeFactor = Number.parseFloat(attrs['custom-ease-factor'] || '2.5')
  const interval = Number.parseInt(attrs['custom-interval'] || '0')
  const repetitions = Number.parseInt(attrs['custom-repetitions'] || '0')
  const nextReview = Number.parseInt(attrs['custom-next-review'] || '0')
  ...
  try {
    ...
    const parseCard = (block: any): FSRSReviewCard => {
      ...
      return {
        ...
        nextReview: Number.parseInt(attrs['custom-next-review'] || '0'),
