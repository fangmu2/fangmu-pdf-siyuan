  ...
  async export(mindMap: MindMap, svgElement: SVGSVGElement, options: ExportOptions): Promise<ExportResult> {
    ...
    try {
      switch (options.format) {
        ...
        default:
          return { success: false, filename, mimeType: '', error: '不支持的导出格式' };
      }
    } catch (error) {
      return {
        success: false,
...
${svgString}`;
  ...
  private async exportPng(svgElement: SVGSVGElement, filename: string, options: ExportOptions): Promise<ExportResult> {
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        if (!ctx) {
          resolve({ success: false, filename, mimeType: '', error: '无法创建 Canvas' });
          return;
        }
        ...
        canvas.toBlob(
          (blob) => {
            if (blob) {
              ...
              resolve({ success: false, filename, mimeType: '', error: 'PNG 生成失败' });
            }
          },
          'image/png',
          quality
      ...
      img.onerror = () => {
        URL.revokeObjectURL(svgUrl);
        resolve({ success: false, filename, mimeType: '', error: '图像加载失败' });
      };
  ...
  private async exportPdf(svgElement: SVGSVGElement, filename: string, options: ExportOptions): Promise<ExportResult> {
    // 先生成 PNG
    const pngResult = await this.exportPng(svgElement, filename, { ...options, format: 'png' });
