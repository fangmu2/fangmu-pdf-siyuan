...
import { useDebounceFn, useThrottleFn, useTimeoutFn, useIntervalFn } from '@vueuse/core';
...
export function createDebounceFn<T extends (...args: any[]) => any>(
  ...
  const { delay = 300, immediate = false, cancelOnUnmount = true } = options;
...
export function createInterval(
  ...
  const { start, stop, pause, resume, isActive } = useIntervalFn(callback, interval, {
    immediate
  });
...
export function createPoll<T>(
  ...
  const { start, stop, pause, resume, isActive } = useIntervalFn(async () => {
    try {
  ...
  return { start, stop, pause, resume, isActive, data, error };
}
...
export { useDebounceFn, useThrottleFn, useTimeoutFn, useIntervalFn };
